...
(function () {
  ...
  function loadAll() {
    ...
    [1,2,3,4,5,6].forEach(function (i) { slot(imgs.run, i - 1, "/sprites/run-" + i + ".png"); });
    [1,2,3,4].forEach(function (i) { slot(imgs.jump, i - 1, "/sprites/jump-" + i + ".png"); });
    [1,2,3,4].forEach(function (i) { slot(imgs.slide, i - 1, "/sprites/slide-" + i + ".png"); });
    [1,2,3,4].forEach(function (i) { slot(imgs.throw, i - 1, "/sprites/throw-" + i + ".png"); });
    [1,2,3,4].forEach(function (i) { slot(imgs.paper, i - 1, "/sprites/paper-" + i + ".png"); });
    one("scoop", "/sprites/scoop.png"); one("breaking", "/sprites/breaking.png"); one("pass", "/sprites/press-pass.png");
    one("camera", "/sprites/camera.png"); one("van", "/sprites/van.png"); one("protestor", "/sprites/protestor.png");
    one("paparazzi", "/sprites/paparazzi.png"); one("scaffolding", "/sprites/scaffolding.png"); one("fakeNews", "/sprites/fake-news.png");
    one("crowd", "/sprites/crowd.png"); one("jetpack", "/sprites/jetpack.png");
    one("politician", "/sprites/politician.png"); one("celebrity", "/sprites/celebrity.png");
    one("mobster", "/sprites/mobster.png"); one("lawyer", "/sprites/lawyer.png"); one("athlete", "/sprites/athlete.png");
    SEASONS.forEach(function (id) {
      jobs.push(loadImg("/bg/" + id + "-sky.jpg").then(function (im) { imgs.sky[id] = im; }));
    });
  ...
  function fillStore() {
    ...
    grid.innerHTML = STORE.map(function (o) {
      return '<article class="obs"><img src="/sprites/' + o.file + '" alt="" />' +
