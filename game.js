(() => {
  ...
  function tick() {
    ...
    for (const s of scoops) {
      ...
      if (dx * dx + dy * dy < (s.r + 16) * (s.r + 16)) {
        s.taken = true;
        score += 1;
        scoreEl.textContent = "SCOOPS " + score;
      }
    }
    draw();
  ...
  function draw() {
    ...
    ctx.fillStyle = "rgba(232,220,200,0.04)";
    for (let i = 0; i < 40; i++) ctx.fillRect((i * 37 + t * 0.4) % W, (i * 53) % H, 2, 2);

    ctx.fillStyle = "#1b140a";
    ctx.fillRect(0, GROUND + 48, W, H);
    ctx.fillStyle = "#f5b942";
    ctx.fillRect(0, GROUND + 48, W, 3);

    ctx.fillStyle = "#e8dcc8";
    ctx.fillRect(player.x, player.y - player.h, player.w, player.h);
    ctx.fillStyle = "#f5b942";
    ctx.fillRect(player.x + 8, player.y - player.h - 10, 20, 10);
    ctx.fillStyle = "#3ee0d4";
    ctx.fillRect(player.x + 22, player.y - 18, 16, 8);
